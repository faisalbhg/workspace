                ...
                <div class="panel p-3">
                    ...
                    <div class="panel-body bio-graph-info" id="subscriptionTop">
                        ...
                        <div class="contanbody p-3">
                            ...
                            <div class="containerbl col-md-6">
                                <section class=left>
                                    @if($subscription->package_category!=2)
                                    <p> {{ $subscription->credits-$subscription->credits_used }}</p>
                                    <p> Credits</p>
                                    @else
                                    <a class="btn-info">Meeting Room</a>
                                    @endif
                                ...
                                <section class="right" wire:click="openBookingSubscription({{$subscription->id}})">
                                    ...
                                    <p class="map-pin"> {{ $subscription->workspaceUnitsDetails['address'] }}</p>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body text-center">
                    ...
                    <p class="h5"><i class="la la-map-marker"></i>{{ $bookingDetails->workspaceUnitsDetails->address }}</p>
                    @if($bookingDetails->package_category!=2)
                    ...
                    @else
                    <p><span class="text-dark h6 text-left">Date: {{ \Carbon\Carbon::parse($bookingDetails->meeting_date)->format('dS F Y') }}</span></p>
                    <p><span class="text-dark h6 text-left">Duration: {{ Config('global.meetinDurations')[$bookingDetails->duration] }}</span></p>
                    <p><span class="text-success text-right">{{ \Carbon\Carbon::parse($bookingDetails->start_time)->format('H:i A') }}</span> - <span class="text-danger text-right">{{ \Carbon\Carbon::parse($bookingDetails->end_time)->format('H:i A') }}</span></p>
                    @endif
                </div>
