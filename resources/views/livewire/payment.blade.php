    ...
    <div class="container">
        <div class="row m-0">
                ...
                <div class="row">
                    <div class="col-12">
                        <div class="box-right">
                            ...
                            @if($showPaymentPannel)
                                <div class="d-none row">
                                    <div class="col-12 mb-2">
                                        <p class="ps-3 text-dark fw-bold h6 mb-2">PAYMENTS</p>
                                        @error('new_card_name')<p><span class="mb-4 text-danger">{{ $message }}</span><br></p>@enderror
                                        @error('new_card_number')<p><span class="mb-4 text-danger">{{ $message }}</span><br></p>@enderror
                                        @error('new_exp_month_year')<p><span class="mb-4 text-danger">{{ $message }}</span><br></p>@enderror
                                        @error('new_card_cvv')<p><span class="mb-4 text-danger">{{ $message }}</span><br></p>@enderror
                                    </div>
                                    <form wire:submit.prevent="saveSubmitPayNow" wire:ignore>
                                        <div class="row">
                                            <div class="col-md-6 containerpayment preload">
                                            ...
                                            <div class="col-md-6 form-container">
                                                <div class="field-container">
                                                    ...
                                                    <input id="name" wire:model="new_card_name" maxlength="20" type="text">
                                                </div>
                                                <div class="field-container">
                                                    <label for="cardnumber">Card Number</label><span id="generatecard" style="display:none;">generate random</span>
                                                    <input id="cardnumber" type="text" pattern="[0-9/]*" wire:model="new_card_number" inputmode="numeric">
                                                ...
                                                <div class="field-container">
                                                    ...
                                                    <input id="expirationdate" type="text" pattern="[0-9]*" wire:model="new_exp_month_year" inputmode="numeric">
                                                </div>
                                                <div class="field-container">
                                                    <label for="securitycode">Security Code</label>
                                                    <input id="securitycode" type="text" pattern="[0-9]*" wire:model="new_card_cvv" inputmode="numeric">
                                                </div>
                                ...
                                <div class=" row pt-0">
                                    ...
                                    <div class="col-5">
                                        ...
                                        <div class="btn btn-primary btn-xl d-block h4" wire:click="proceedPaymentSubmit">
                                            @if($this->grand_total_price==0)
                                            Complete
                                            @else
                                            Proceed <span class=" ms-2">{{config('global.CURRENCY')}}</span>{{$this->grand_total_price}} <span class="ms-3 fas fa-arrow-right"></span>
                                            @endif
                                        </div> 

                                        <div wire:loading wire:target="proceedPaymentSubmit">
            ...
            <div class="col-md-5 col-12 ps-md-5 p-0 ">
                <div class="box-left">
                    <p class="ps-3 text-dark fw-bold h6 mb-4">PAYMENT SUMMARY</p>
                    <div class="h8">
                        <div class="row m-0 border mb-3">
                            <div class="col-6 h8 pe-0 ps-2">
                                <h5 class="text-dark py-2">Items</h5>
                                <span class="d-block py-2 border-bottom">{{$tempSubscriptions->package_name}}</span>
                                @if($tempSubscriptions->parking)<span class="d-block py-2 border-bottom">Parking</span>@endif
                                <span class="d-block py-2">Vat</span>
                                @if($show_discount)
                                <span class="d-block py-2">Discount</span>
                                @endif
                            </div>

                            <div class="col-3 text-center p-0">
                                <h5 class="text-dark p-2">Qty</h5>
                                <span class="d-block p-2 border-bottom text-dark">1</span>
                                @if($tempSubscriptions->parking)
                                <span class="d-block p-2 border-bottom text-dark">{{$tempSubscriptions->no_of_parking}}</span>
                                @endif
                                <span class="d-block p-2"></span>
                            </div>
                            <div class="col-3 p-0 text-center h8 border-end">
                                <h5 class="text-dark p-2">Price</h5>
                                <span class="d-block border-bottom py-2 text-dark">{{config('global.CURRENCY').$this->item_total_price}}</span>
                                @if($tempSubscriptions->parking)
                                <span class="d-block border-bottom py-2 text-dark">{{config('global.CURRENCY').$tempSubscriptions->parking_charge}}</span>
                                @endif
                                <span class="d-block py-2 text-dark">
                                    {{config('global.CURRENCY').$this->vat_price}}
                                </span>
                                @if($show_discount)
                                <span class="d-block py-2">{{config('global.CURRENCY').$discount_amount}}</span>
                                @endif
                            </div>
                        </div>

                        @if(!empty($listPaymentCards))
                            @foreach($listPaymentCards as $listPaymentCard)
                            <div class="d-none row">
                                <div class="col-6">
                                    <p class="h7">xxxx xxxx xxxx {{substr ($listPaymentCard->payment_card_number, -4)}}</p>
                                </div>
                                <div class="col-3 px-0">
                                    <input class="form-control @error('default_card_cvv.'.$listPaymentCard->id) border-danger @enderror" type="password" wire:model="default_card_cvv.{{$listPaymentCard->id}}" placeholder="CVV" @if(!$listPaymentCard->default_card) disabled @endif >
                                </div>
                                <div class="col-3 p-2">
                                    <input class="form-control" type="radio" name="default_payment" wire:model="default_payment.{{$listPaymentCard->id}}" wire:click="defaultPaymentOption('{{$listPaymentCard->id}}')" value="1" >
                                </div>
                            </div>
                            @endforeach
                            <div class="d-none w-100 h7 mt-4 mb-2" class="float-end">
                                <p class="text-dark">Grand Total: </p><p class="h3 fw-bold d-flex" style="margin-left: auto;"> <span class="text-dark pe-1 h6 align-text-top mt-1"> {{config('global.CURRENCY')}}</span> <span class="text-dark">{{$this->grand_total_price}}</span> </p>

                            </div>
                            <div class="d-none w-100 h7 mt-4 mb-2 float-end">
                                @if($default_payment_id)
                                <p class="h3 fw-bold d-flex" style="margin-left: auto;">
                                    <div class="btn btn-primary d-block h8 " wire:click="payDefaultCardSubmit('{{$default_payment_id}}')">PAY <span class=" ms-2">{{config('global.CURRENCY')}}</span>{{$tempSubscriptions->grand_total}} <span class="ms-3 fas fa-arrow-right"></span></div>
                                    <div wire:loading wire:target="payDefaultCardSubmit">
                                        <div style="display: flex; justify-content: center; align-items: center; background-color: black; position: fixed; top: 0px; left: 0px; z-index:999999; width:100%; height:100%; opacity: .75;" >
                                            <div class="la-ball-beat">
                                                <div></div>
                                                <div></div>
                        ...
                        @endif
                        <div class="d-flex w-100 h7 mt-4 mb-2" class="float-end">
                            <p class="text-dark">Grand Total: </p><p class="h3 fw-bold d-flex" style="margin-left: auto;"> <span class="text-dark pe-1 h6 align-text-top mt-1"> {{config('global.CURRENCY')}}</span> <span class="text-dark">{{$this->grand_total_price}}</span> </p>
...
<script type="text/javascript">
    window.addEventListener('paymentPannel',event=>{
